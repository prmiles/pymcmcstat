...
class CheckDependentOptions(unittest.TestCase):
    ...
    def test_lastadapt(self):
        ...
        SO._check_dependent_simulation_options(model=MS)
        self.assertEqual(SO.lastadapt, SO.nsimu,
                         msg='lastadapt set to nsimu')
        SO.define_simulation_options(lastadapt=10)
        SO._check_dependent_simulation_options(model=MS)
        self.assertEqual(SO.lastadapt, 10,
                         msg='lastadapt unchanged')
        
    def test_printint(self):
        SO = setup_options(printint = None)
        SO._check_dependent_simulation_options(model=MS)
        self.assertEqual(SO.printint, max(100,min(1000,SO.adaptint)),
                         msg='printint was updated')
        
    def test_updatesigma(self):
        SO = setup_options(updatesigma=False)
        MS.N0 = [1.]
        SO._check_dependent_simulation_options(flag=MS.N0 is not None)
        self.assertTrue(SO.updatesigma,
                        msg='updatesigma turned on because N0 not empty')
        SO = setup_options(updatesigma=False)
        MS.N0 = None
        SO._check_dependent_simulation_options(flag=MS.N0 is not None)
        self.assertFalse(SO.updatesigma,
                        msg='updatesigma turned off because N0 empty')
