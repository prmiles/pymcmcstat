...
class PredictionIntervals:
    ...
    def setup_prediction_interval_calculation(self, results, data, modelfunction, burnin=0):
        ...
        self.datapred = self._setup_data_structure_for_prediction(
                data=data, ndatabatches=self.__ndatabatches)
        ...
        self.__nrow, self.__ncol = self._determine_shape_of_response(
                modelfunction=modelfunction, ndatabatches=self.__ndatabatches,
                datapred=self.datapred, theta=self.__theta)
        # analyze structure of s2chain with respect to model output
        if self.__s2chain is not None:
            self.__s2chain_index = self._analyze_s2chain(
                    ndatabatches=self.__ndatabatches, s2chain=self.__s2chain, ncol=self.__ncol)
    ...
    def generate_prediction_intervals(self, sstype=None, nsample=500, calc_pred_int=True, waitbar=False):
        ...
        '''
        chain, s2chain, lims, sstype, nsample, iisample = self._setup_generation_requirements(
                sstype=sstype, nsample=nsample, calc_pred_int=calc_pred_int)
        # setup progress bar
        ...
        if s2chain is None:
            ...
            credible_intervals, prediction_intervals = self._calculate_ci_and_pi_for_data_sets(
                testchain=testchain, s2chain=s2chain, iisample=iisample, waitbar=waitbar,
                sstype=sstype, lims=lims)
    ...
    def _setup_display_settings(cls, interval_display, model_display, data_display):
        ...
        # Setup model display
        default_model_display = {'linestyle': '-', 'color': 'r', 'marker': '',
                                 'linewidth': 2, 'markersize': 5, 'label': 'model'}
        model_display = check_settings(default_model_display, model_display)
        # Setup data display
        default_data_display = {'linestyle': '', 'color': 'b', 'marker': '.',
                                'linewidth': 1, 'markersize': 5, 'label': 'data'}
    ...
    def _setup_interval_plotting(self, plot_pred_int, prediction_intervals, credible_intervals, figsizeinches):
        ...
        '''
        prediction_intervals = self._check_prediction_interval_flag(
                plot_pred_int=plot_pred_int, prediction_intervals=prediction_intervals)
